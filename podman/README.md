...
This variant is built and run with **Podman**. Secrets are stored in Podman's secret store and injected into the container at runtime as environment variables. Claude Desktop's configuration file references secrets by name only — it never contains the API key, hostname, or any other credential.

> Looking for the Docker version? It's in the parent directory and uses the Docker MCP Toolkit.
...
- `.containerignore`
- `requirements.txt`
- `.env.example` (reference only — not used by the container)
...
### Step 4 — Store Secrets in Podman (not in Claude Desktop)
...
- **Never store the API key in Claude Desktop's config file.** All credential storage goes through `podman secret create`.
...
- Your Podman is older than 4.4. Upgrade Podman, or fall back to `--env-file /path/to/file` and store secrets in a 600-permissioned file outside Claude Desktop's config directory.
